...
humanDimensionWalk = 1+2+3+6+6*8+3*8  #=84
humanDimensionWalkOutput = humanDimensionWalk - 3
objDimention = 83 * 2 + 1  #36  fake now
...
class trainDataLoader:
    ...
    def updateNorm(self):
        ...
        #temp_obj /= 3.14159265
        #temp_whole = np.array(self.skeleton_data)[:, :, 3:].flatten()
            ...
            def handleSingleFile(start, end, step):
                ...
                with open(f, 'rb') as pickle_file:
                    ...
                    for i in range(start, end, step):
                        ...
                        JI = JointsInfo(bodyWholePos)  #centered at Hip
                ...
                bodyResultOnly = bodyDataSave[:, 2:]
                self.skeletonData.append(bodyResultOnly)

                markerPos1 = np.einsum("a,ab->ab", phase_angle_sin, bodyDataSave)
                markerPos2 = np.einsum("a,ab->ab", phase_angle_cos, bodyDataSave)
                
                markerPosTemp = np.concatenate(( np.array(delta_phase).reshape(85, 1), markerPos1, markerPos2), axis=1)
                #self.skeletonData.append(markerPosTemp)
                
                #markerPos = np.concatenate((markerPos1, markerPos2), axis=-1)
                self.objectMarkerData.append(markerPosTemp)
    ...
    def prepareDataset(self):
        ...
        for file_count in range(len(self.skeletonData)):
            pos_zeros = np.zeros((max_length, humanDimensionWalkOutput))
        ...
        for i in range(len(self.obj_data)):
            ...
            #self.skeleton_data[i][:, 3:] = (self.skeleton_data[i][:, 3:] - self.wholeMean) / self.wholeStd
    ...
    def getDataset3(self):
        ...
        def splitDataToY(toBeSplit):
            toBeSplit = np.array(toBeSplit)
            theX = toBeSplit[:, 1:, :]
            toBeSplit = toBeSplit[:, :, 3:]
            theY = toBeSplit[:, :-1, :]
            Xshape = theX.shape
            Yshape = theY.shape
            return theX.reshape(Xshape[0] * Xshape[1], Xshape[2]), theY.reshape(Yshape[0] * Yshape[1], Yshape[2])
        
        def re_arrange(thisX, thisY):
            resultX = []
            resultY = []
            for fileCount, fileX in enumerate(thisX):
                fileY = thisY[fileCount]
                for singleCount in range(85):
                    resultX.append(fileX[singleCount])
                    resultY.append(fileY[singleCount])
        ...
        #currentX, currentY = splitDataToY(self.skeleton_data)
        currentX, currentY = self.objectMarkerData, self.skeleton_data
        currentX, currentY = re_arrange(currentX, currentY)
        #dataset = tf.data.Dataset.from_tensor_slices((self.obj_data, self.skeleton_data))
        dataset = tf.data.Dataset.from_tensor_slices((currentX, currentY))
        return dataset
