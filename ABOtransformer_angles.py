...
class Transformer_initalPose(Transformer_newScheduleSampling):
  def __init__(
        ...
        self.initalPosStart3 = layers.Dense(114, name="intial_human")  #63  54  54+63-3=114
...
class Transformer_VAEinitalPose(Transformer_initalPose):
  def __init__(
        ...
        self.use_schedule_sampleing_two_runs = True
        if self.scheule_sampling_gt_rate < 1:
          self.use_schedule_sampleing_two_runs = False
  ...
  def setDiscriminator(self, model_using, weight_path = None):
    self.discriminator_model = model_using
    self.discriminator_model.load_weights(weight_path)
  ...
  def call(self, inputs):
    ...
    initalPos = self.initalPosStart3(s2)
    '''
    initalPos = self.getInitialPos(source[:,:,:])
    target = tf.concat((tf.expand_dims(initalPos, axis=1), target[:, 1:, :]), axis=1)
    ...
    #return result, initalPos, kl_loss
  ...
  def train_step(self, batch):
    ...
    y = tf.concat((y, obj_center), axis = -1)
    y_predFinal = None
    if self.use_schedule_sampleing_two_runs:
      y_pred1, _, _ = self([source, dec_input], training=False)
      new_input = tf.concat((tf.expand_dims(dec_input[:,0,:], axis=1), y_pred1[:,1:,:]), axis=1)
      new_input = self.mix_Data(dec_input, new_input, self.scheule_sampling_gt_rate)
      #print(new_input.shape) (4,84,114)
      mask_global_pos_1 = tf.ones(shape=(batch_size, 1, 111))
      mask_global_pos_0 = tf.zeros(shape=(batch_size, 1, 3))
      mask_global_pos = tf.concat((mask_global_pos_0, mask_global_pos_1), axis=-1)
      new_input_init = tf.expand_dims(new_input[:, 0, :], axis=1) * mask_global_pos
      new_input = tf.concat((new_input_init, new_input[:, 1:, :]), axis=-2)
      #exit()
      with tf.GradientTape() as tape:
        y_pred2, initalPos, losses = self([source, new_input], training=True)
        kl_loss, firstTwoFrameDiff = losses
        loss2 = self.compiled_loss(y_true={"final_result": y, "intial_human":y_initalPos}, y_pred={"final_result":y_pred2, "intial_human":initalPos})
        loss = loss2 + kl_loss + firstTwoFrameDiff
        if self.discriminator_model is not None:
          allposes = y_pred2[:, :, 3:]
          #allposes = tf.concat((tf.expand_dims(initalPos, axis=1)[:, :, 3:], allposes), axis=1)
          loss_discriminator = tf.reduce_sum(self.discriminator_model(allposes))
          loss += loss_discriminator
      y_predFinal = y_pred2
    else:
      with tf.GradientTape() as tape:
        y_pred1, initalPos, losses= self([source, dec_input], training=True)
        kl_loss, firstTwoFrameDiff = losses
        loss2 = self.compiled_loss(y_true={"final_result": y, "intial_human":y_initalPos}, y_pred={"final_result":y_pred1, "intial_human":initalPos})
        loss = loss2 + kl_loss + firstTwoFrameDiff
        if self.discriminator_model is not None:
          allposes = y_pred1[:, :, 3:]
          #allposes = tf.concat((tf.expand_dims(initalPos, axis=1)[:, :, 3:], allposes), axis=1)
    ...
    gradients = tape.gradient(loss, trainable_vars)
    self.optimizer.apply_gradients(zip(gradients, trainable_vars))
    self.compiled_metrics.update_state(y_true={"final_result": y, "intial_human":y_initalPos}, y_pred={"final_result":y_predFinal, "intial_human":initalPos})
    return {m.name: m.result() for m in self.metrics}

  def generate(self, source, dec_start_input_GT=None, dec_Pre= None, loader=None):
    ...
    mask_global_pos_0 = tf.zeros(shape=(batch_size, 1, 3))
    mask_global_pos = tf.concat((mask_global_pos_0, mask_global_pos_1), axis=-1)
    dec_start_input = dec_start_input * mask_global_pos
    #new_input = tf.concat((new_input_init, new_input[:, 1:, 0]), axis=-2)
    ...
    z_mean = x[:, 0, :]
    z_log_var = x[:, 1, :]
    sample = self.sampling(z_mean, z_log_var, True)
    sample = tf.tile(tf.expand_dims(sample, axis=1), [1, self.target_maxlen, 1])
    ...
    for i in range(self.target_maxlen - 1):
      ...
      outCentered = out[:,-60:].numpy() * loader.ppl_std_p  + loader.ppl_mean_p #+ out[:,:3].numpy()
      humanCenter = out[:,:3].numpy()

      #6-7-8  10-11-12
      indexOffset = -3 #51 #3 + 17*3 - 3
      shoulder1, shoulder2 = (outCentered[0, indexOffset + 6*3: indexOffset + 6*3+3], outCentered[0, indexOffset + 10*3: indexOffset + 10*3+3])
      elbow1, elbow2 = (outCentered[0, indexOffset + 7*3: indexOffset + 7*3+3], outCentered[0, indexOffset + 11*3:indexOffset + 11*3+3])
      hand1, hand2 = (outCentered[0, indexOffset + 8*3:indexOffset + 8*3+3], outCentered[0, indexOffset + 12*3:indexOffset + 12*3+3])
      newelbow1, newhand1 = thisIK.solver(source[0, i, 11*3:11*3+3] - humanCenter, shoulder1, elbow1, hand1)
      newelbow2, newhand2 = thisIK.solver(source[0, i, 9*3:9*3+3] - humanCenter, shoulder2, elbow2, hand2)
      outCentered[0,indexOffset + 7*3: indexOffset + 7*3+3], outCentered[0,indexOffset + 8*3: indexOffset + 8*3+3] = (newelbow1, newhand1)
      outCentered[0,indexOffset + 11*3: indexOffset + 11*3+3], outCentered[0,indexOffset + 12*3: indexOffset + 12*3+3] = (newelbow2, newhand2)
      out_last60 = tf.convert_to_tensor((outCentered - loader.ppl_mean_p) / loader.ppl_std_p, dtype=tf.float32)
      out = tf.concat([out[:, :-60], out_last60], axis=-1)
      '''   #end of IK
    ...
    print("++++++++")
    #theLoss = self.compiled_loss(y_true={"final_result": dec_Pre, "intial_human":dec_start_input_GT}, y_pred={"final_result":result, "intial_human":initalPos})
    #print(theLoss)
    return result
  ...
  def predict_step(self, batch):
    ...
    #with tf.GradientTape() as tape:
    y_pred1, _, _ = self([source, dec_input], training=False)
    if not self.use_schedule_sampleing_two_runs:
      return y_pred1
    #loss1 = self.compiled_loss(y, y_pred1)
    #new_input = tf.concat((tf.expand_dims(dec_input[:,0,:], axis=1), y_pred1[:,:-1,:]), axis=1)
    new_input = tf.concat((tf.expand_dims(dec_input[:,0,:], axis=1), y_pred1[:,1:,:]), axis=1)
